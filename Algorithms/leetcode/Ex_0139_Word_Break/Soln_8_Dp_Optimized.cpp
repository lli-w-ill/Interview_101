    ...
    bool wordBreak(string s, vector<string>& wordDict) {
        ...
        for (int i = 1; i <= lenS; i++) {   // i: s子串的结束位置（不包含i），即到s[0 ...i-1] 或 s[0 ... i)
            for (int j = i - 1; j >= 0; j--) {  // j: 在 0 ... i 中切分子串s [0 ... i)
                // 优化点1: 当发现 s[0 ... i) 已经计算过可以划分后，就不用再重新计算了
                if (dp[i] == true) {
                    break;
                }
                // 优化点2: 当发现 s[0 ...j) 不能划分后，那么dp[i]不可能为true，尝试下一个j，即检查 s[0 ... j+1) 是否能划分
                if (dp[j] == false) {
                    continue;
                }
                ...
                // dp[i] 取决于 2. 剩余子串 s[j ... i) 是否是 wordSet 中的一个单词（见最后「状态转移方程」）
                if (wordSet.find(sub) != wordSet.end()) {
                    // ok, s[j ... i) 是一个单词，接下来
                    // dp[i] 取决于 1. dp[j] 是否为true，即 s[0 ... j) 是否可以拆分（见最后「状态转移方程」）
                    if (dp[j] == true) {
